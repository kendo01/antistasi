...
BTN_M(BTN_Y_5, 112, "Buy APC", "", "if (activeAFRF) then {if (player == Dolvich) then {closeDialog 0; [vfs select 11] call addFIAveh;} else {hint ""Only Player Commander has access to this function""};}else {hint ""RHS exclusive for now""};");
...
#define STR_LOAD_YES "closeDialog 0; if ((player == Dolvich) and (isNil ""placementDone"")) then {[""statSave\loadAccount.sqf"",""BIS_fnc_execVM""] call BIS_fnc_MP; placementDone = true; publicVariable ""placementDone""} else {[] execVM ""statSave\loadAccount.sqf"";};"
...
BTN_BACK("closeDialog 0; if (player == Dolvich) then {createDialog ""radio_comm_commander""} else {createDialog ""radio_comm_player""};");
...
#define STR_MIS_MIL "closeDialog 0; if ((player == Dolvich) or (not(isPlayer Dolvich))) then {[[""FND_M""],""missionrequest""] call BIS_fnc_MP} else {hint ""Only Player Commander has access to this function""};"
#define STR_MIS_CIV "closeDialog 0; if ((player == Dolvich) or (not(isPlayer Dolvich))) then {[[""FND_C""],""missionrequest""] call BIS_fnc_MP} else {hint ""Only Player Commander has access to this function""};"
#define STR_MIS_EXP "closeDialog 0; if ((player == Dolvich) or (not(isPlayer Dolvich))) then {[[""FND_E""],""missionrequest""] call BIS_fnc_MP} else {hint ""Only Player Commander has access to this function""};"
#define STR_MIS_LOG	"closeDialog 0; if ((player == Dolvich) or (not(isPlayer Dolvich))) then {[[""LOG""],""missionrequest""] call BIS_fnc_MP} else {hint ""Only Player Commander has access to this function""};"
#define STR_MIS_DES	"closeDialog 0; if ((player == Dolvich) or (not(isPlayer Dolvich))) then {[[""DES""],""missionrequest""] call BIS_fnc_MP} else {hint ""Only Player Commander has access to this function""};"
#define STR_MIS_RES "closeDialog 0; if ((player == Dolvich) or (not(isPlayer Dolvich))) then {[[""RES""],""missionrequest""] call BIS_fnc_MP} else {hint ""Only Player Commander has access to this function""};"
#define STR_MIS_PRO "closeDialog 0; if ((player == Dolvich) or (not(isPlayer Dolvich))) then {[[""PR"",false,true],""missionrequest""] call BIS_fnc_MP} else {hint ""Only Player Commander has access to this function""};"
...
BTN_R1(-1, "Vehicles and Squads", "", "closeDialog 0; if (player == Dolvich) then {createDialog ""squad_manager""} else {hint ""Only Player Commander has access to this function""};");

BTN_M(BTN_Y_2, -1, "Unlock Vehicle", "", "closeDialog 0; if !(isMultiplayer) then {hint ""It's unlocked already.""} else {if (player != Dolvich) then {[false] call AS_fnc_unlockVehicle} else {[true] call AS_fnc_unlockVehicle};};");
...
BTN_L1(-1, "Garage Vehicle", "", "closeDialog 0; if (player != Dolvich) then {[false] call AS_fnc_garageVehicle} else {if (isMultiplayer) then {createDialog ""garage_check""} else {[true] call AS_fnc_garageVehicle}};");
BTN_R1(-1, "Sell Vehicle", "", "closeDialog 0; if (player == Dolvich) then {[] call AS_fnc_sellVehicle} else {hint ""Only the Commander can sell vehicles""};");
...
class com_menu // 360
{
	...
	#define STR_COM_FIA "closeDialog 0; if (player == Dolvich) then {[""status""] remoteExecCall [""AS_fnc_infoScreen"", 2]};"
...
class boost_menu // 390
{
	...
	#define STR_BST_YES "closeDialog 0; if (player == Dolvich) then {[[], ""boost.sqf""] remoteExec [""execVM"", 2];};if ((player == Dolvich) and (isNil ""placementDone"")) then {[] spawn placementselection};"
	#define STR_BST_NO "closeDialog 0; [false] remoteExec [""AS_fnc_MAINT_arsenal"", 2]; if (activeBE) then {[] call fnc_BE_refresh}; if ((player == Dolvich) and (isNil ""placementDone"")) then {[] spawn placementselection};"
...
class misCiv_menu // 400
{
	...
	#define STR_CIV_ASS "closeDialog 0; if (((getPlayerUID player) in miembros) || (player == Dolvich)) then {[[""ASS""],""misReqCiv""] call BIS_fnc_MP} else {hint ""Stranger does not trust you.""};"
	#define STR_CIV_CVY "closeDialog 0; if (((getPlayerUID player) in miembros) || (player == Dolvich)) then {[[""CONVOY""],""misReqCiv""] call BIS_fnc_MP} else {hint ""Stranger does not trust you.""};"
	#define STR_CIV_CON "closeDialog 0; if (((getPlayerUID player) in miembros) || (player == Dolvich)) then {[[""CON""],""misReqCiv""] call BIS_fnc_MP} else {hint ""Stranger does not trust you.""};"
...
class misMil_menu // 410
{
	...
	#define STR_MIL_ASS "closeDialog 0; if (((getPlayerUID player) in miembros) || (player == Dolvich)) then {[[""AS""],""misReqMil""] call BIS_fnc_MP} else {hint ""Nomad does not trust you.""};"
	#define STR_MIL_CVY "closeDialog 0; if (((getPlayerUID player) in miembros) || (player == Dolvich)) then {[[""CONVOY""],""misReqMil""] call BIS_fnc_MP} else {hint ""Nomad does not trust you.""};"
	#define STR_MIL_CON "closeDialog 0; if (((getPlayerUID player) in miembros) || (player == Dolvich)) then {[[""CON""],""misReqMil""] call BIS_fnc_MP} else {hint ""Nomad does not trust you.""};"
	#define STR_MIL_DES "closeDialog 0; if (((getPlayerUID player) in miembros) || (player == Dolvich)) then {[[""DES""],""misReqMil""] call BIS_fnc_MP} else {hint ""Nomad does not trust you.""};"
...
class exp_menu // 430
{
	...
	#define STR_EXP_SCH "closeDialog 0; if (player == Dolvich) then {[expCrate, ""expLight"", 300] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_MCH "closeDialog 0; if (player == Dolvich) then {[expCrate, ""expLight"", 800] remoteExec [""buyGear"", 2];}"

	#define STR_EXP_SWP "closeDialog 0; createDialog ""wpns_small"";"
	#define STR_EXP_MWP "closeDialog 0; createDialog ""wpns_large"";"

	#define STR_EXP_SMS "closeDialog 0; if (player == Dolvich) then {[expCrate, ""expHeavy"", 300] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_MMS "closeDialog 0; if (player == Dolvich) then {[expCrate, ""expHeavy"", 800] remoteExec [""buyGear"", 2];}"

	#define STR_EXP_SAC "closeDialog 0; if (player == Dolvich) then {[expCrate, ""aCache"", 500] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_MAC "closeDialog 0; if (player == Dolvich) then {[expCrate, ""aCache"", 5000] remoteExec [""buyGear"", 2];}"
...
class wpns_small
{
	...
	#define STR_EXP_ASS_S "closeDialog 0; if (player == Dolvich) then {[expCrate, ""ASRifles"", 1000] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_PIS_S "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Pistols"", 1000] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_MGS_S "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Machineguns"", 1000] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_SNP_S "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Sniper Rifles"", 1000] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_LCH_S "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Launchers"", 1000] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_RND_S "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Random"", 1000] remoteExec [""buyGear"", 2];}"
...
class wpns_large
{
	...
	#define STR_EXP_ASS_L "closeDialog 0; if (player == Dolvich) then {[expCrate, ""ASRifles"", 2500] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_PIS_L "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Pistols"", 2500] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_MGS_L "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Machineguns"", 2500] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_SNP_L "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Sniper Rifles"", 2500] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_LCH_L "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Launchers"", 2500] remoteExec [""buyGear"", 2];}"
	#define STR_EXP_RND_L "closeDialog 0; if (player == Dolvich) then {[expCrate, ""Random"", 2500] remoteExec [""buyGear"", 2];}"
...
#define STR_GO_RSA "closeDialog 0; if (player == Dolvich) then {[true] remoteExec [""AS_fnc_MAINT_arsenal"", 2];};"
...
class maintenance_menu
{
	...
	#define STR_MAINT_MOV "closeDialog 0; [] remoteExec ['AS_fnc_addMoveObjAction',Dolvich];"
