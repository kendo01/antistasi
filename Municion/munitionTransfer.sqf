			...
			/*
				...
				if (typeName _cosa == typeName []) then
					...
					if (count _cosa > 0) then
						...
						if (!isNil "_subCosa") then {_municion pushBack _subCosa; Dolvich sidechat format ["%1,%2",_municion,_subCosa];} else {diag_log format ["Error transfering ammo on %1",_cosa]};
						};
					};
				}
			*/
