...
_data = [_airport, _targetPosition, _targetMarker, _duration, "transport", _QRFsize, "campQRF", true] remoteExec ["enemyQRF", 2];

// wait for the QRF script to be loaded
sleep 10;

_soldados = _data select 0;
...
[format ["DefCamp -- Soldiers: %1; Number: %2; campQRF: %3", _soldados, count _soldados, server getVariable ["campQRF", "none"]]] remoteExec ["AS_fnc_logOutput", 2];
...
[format ["DefCamp -- con 1: %1; con 2: %2; con 3: %3; con 4: %4", ({not (captive _x)} count _soldados > {captive _x} count _soldados), ({alive _x} count _soldados > {fleeing _x} count _soldados), ({alive _x} count _soldados > 0), (_targetMarker in campsFIA)]] remoteExec ["AS_fnc_logOutput", 2];
