...
for "_i" from 0 to _cuenta do
	{
	_unit = _grpPOW createUnit [guer_POW, (_poscasa select _i), [], 0, "NONE"];
	...
	_POWS = _POWS + [_unit];
	[[_unit,"prisionero"],"AS_fnc_addActionMP"] call BIS_fnc_MP;
	};
...
waitUntil {sleep 1; ({alive _x} count _POWs == 0) or ({(alive _x) and (_x distance getMarkerPos guer_respawn < 50)} count _POWs > 0) or (dateToNumber date > _fechalimnum)};
...
if ({(alive _x) and (_x distance getMarkerPos guer_respawn < 50)} count _POWs > 0) then
	...
	[round (_cuenta/2),Dolvich] call playerScoreAdd;
