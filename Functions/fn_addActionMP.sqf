...
switch _type do {
	...
	case "moveObject" : {_object addAction [localize "STR_ACT_MOVEASSET", {[_this select 0,_this select 1,_this select 2] spawn AS_fnc_moveObject},nil,0,false,true,"","(_this == Dolvich)"]};
	case "deploy" : {_object addAction [localize "STR_ACT_BUILDPAD", {[_this select 0, _this select 1] remoteExec ["AS_fnc_deployPad", 2]},nil,0,false,true,"","(_this == Dolvich)"]};
