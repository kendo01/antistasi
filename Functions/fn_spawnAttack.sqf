...

{
	...
	if (_objective in ciudades) then {
		...
		if (!(_base == "") or !(_airport == "")) then {
			...
			//Dolvich globalChat format ["Marcador: %1. ScoreNeededLand: %2. ScoreLand: %3. ScoreNeededAir: %4. ScoreAir: %5",_objective,_scoreNeededLand,_scoreLand,_scoreNeededAir,_scoreAir]; sleep 5;
...
if ((count _objectives > 0) and (_difficulty < 3)) then {
	...
	if !(_objective in ciudades) then {[_objective] remoteExec ["combinedCA",HCattack]} else {[_objective] remoteExec ["CSATpunish",HCattack]};
	cuentaCA = cuentaCA - 600; //experimental
};
