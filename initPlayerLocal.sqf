...
if (isMultiplayer) then
	...
	Dolvich = player;
	grupo = group player;
	grupo setGroupId ["Dolvich","GroupColor4"];
...
if (player!=Dolvich) then {player setVariable ["score", 0,true]} else {player setVariable ["score", 25,true]};
...
player addEventHandler ["WeaponAssembled",{
	...
	if (_EHunit isKindOf "StaticWeapon") then {
		_EHobj addAction [localize "STR_ACT_MOVEASSET", {[_this select 0,_this select 1,_this select 2,"static"] spawn AS_fnc_moveObject},nil,0,false,true,"","(_this == Dolvich)"];
...
if (_isJip) then
	...
	if ((player == Dolvich) and (isNil "placementDone") and (isMultiplayer)) then {
...
caja addAction [localize "STR_ACT_MOVEASSET", {[_this select 0,_this select 1,_this select 2] spawn AS_fnc_moveObject},nil,0,false,true,"","(_this == Dolvich)"];
//caja addAction [localize "STR_ACT_SELLMENU", "UI\sellMenu.sqf",nil,0,false,true,"","(_this == Dolvich)", 5];
