...
if (visiblemap) then
	...
	_pos = [getPos fuego, 3, _rnd] call BIS_Fnc_relPos;
	cajaVeh setPos _pos;
	if (isNil "placementDone") then {if (isMultiplayer) then {{_x setPos getPos petros} forEach playableUnits} else {Dolvich setPos (getMarkerPos guer_respawn)}} else {Dolvich allowDamage true};
