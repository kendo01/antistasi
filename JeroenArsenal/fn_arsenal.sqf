...
switch _mode do {
	...
	case "customEvents":{
		...
		_display displayAddEventHandler ["keydown",{['KeyDown',_this] call jna_fnc_arsenal;}];
		...
		_ctrlButtonClose ctrladdeventhandler ["buttonclick",{["buttonClose",[ctrlparent (_this select 0)]] call jna_fnc_arsenal;}];
		...
		_ctrlButtonLoad ctrladdeventhandler ["buttonclick",{["buttonLoad",[ctrlparent (_this select 0)]] call jna_fnc_arsenal;}];
		...
		_ctrlTemplateButtonDelete ctrladdeventhandler ["buttonclick",{["buttonTemplateDelete",[ctrlparent (_this select 0)]] call jna_fnc_arsenal;}];
		...
		_ctrlButtonSave ctrladdeventhandler ["buttonclick",{["buttonSave",[ctrlparent (_this select 0)]] call jna_fnc_arsenal;}];
		...
		_ctrlButtonRandom ctrladdeventhandler ["buttonclick",{["buttonRepackMagazines",[ctrlparent (_this select 0)]] call jna_fnc_arsenal;}];
		...
		_ctrlArrowLeft ctrladdeventhandler ["buttonclick",{["buttonCargo",[ctrlparent (_this select 0),-1]] call jna_fnc_arsenal;}];
		...
		//--- Menus
		_sortValues = uinamespace getvariable ["jna_fnc_arsenal_sort",[]];
		{
			_idc = _x;
			...
			_ctrlList ctrlAddEventHandler ["LBSelChanged",	format ["
				if(uiNamespace getvariable ['jna_userInput',false])then{
					['SelectItem',[ctrlparent (_this select 0),(_this select 0),%1]] call jna_fnc_arsenal;
			...
			_ctrlTab = _display displayctrl (IDC_RSCDISPLAYARSENAL_TAB + _idc);
			{
				...
				if (_idc in [IDCS_LEFT]) then {
					_x ctrladdeventhandler ["buttonclick",format ["['TabSelectLeft',[ctrlparent (_this select 0),%1],true] call jna_fnc_arsenal;",_idc]];
				} else {
					_x ctrladdeventhandler ["buttonclick",format ["['TabSelectRight',[ctrlparent (_this select 0),%1],true] call jna_fnc_arsenal;",_idc]];
				};
			...
			_ctrlSort ctrladdeventhandler ["lbselchanged",format ["['lbSort',[_this,%1]] call jna_fnc_arsenal;",_idc]];
		...
		uinamespace setvariable ["jna_fnc_arsenal_sort",_sortValues];
	};
	...
	case "TabSelectLeft": {
		...
		//create list
		["UpdateListGui",[ _display, _index]] call jna_fnc_arsenal;
		...
		//Select right tab
		if (_showItems) then {
			['TabSelectRight',[_display,IDC_RSCDISPLAYARSENAL_TAB_ITEMOPTIC]] call jna_fnc_arsenal;
		};
		...
		if (_showCargo) then {
			...
			['TabSelectRight',[_display, IDC_RSCDISPLAYARSENAL_TAB_CARGOMAG]] call jna_fnc_arsenal;
		};
	...
	case "TabSelectRight": {
		...
		["CreateList",[ _display, _index, _inventory]] call jna_fnc_arsenal;
		switch _index do {
			...
			case IDC_RSCDISPLAYARSENAL_TAB_ITEMBIPOD: {
				...
				["UpdateItemAdd",[_index,_item,0]] call jna_fnc_arsenal;
				["ListSelectCurrent",[_display,_index,_item]] call jna_fnc_arsenal;
			};
			...
			case IDC_RSCDISPLAYARSENAL_TAB_CARGOMISC:{
				...
				_itemsUnique = [];
				{
					_type = _x call jna_fnc_itemType;
					...
					{
						{
							if(_x in _itemsUnique)then{
								["UpdateItemAdd",[_index,_x,0]] call jna_fnc_arsenal;
							}
						} forEach (getarray (configfile >> "cfgweapons" >> _x >> "magazines"));
					}forEach [primaryweapon player, secondaryweapon player, handgunweapon player];
				}else{
...

		{
			...
			if (_active) then {
				...
				if (
					...
					["SelectItemRight",[_display,_ctrlList,_idc]] call jna_fnc_arsenal;
				};
			};
	...
	case "CreateListAll":{
		...
		_inventory_box_all = jna_dataList;
		{
			...
			if(_index in [IDCS_LEFT])then{
				...
				["CreateList",[_display,_index,_inventory_box]] call jna_fnc_arsenal;
			};
	...
	case "CreateList":{
		...
		//fill
		{
			_item = _x select 0;
			_amount = _x select 1;
			["CreateItem",[_display,_index,_item,_amount]] call jna_fnc_arsenal;
		} forEach _inventory;

		//TODO better sorting of scopes gray items?
		["ListSort",[_display,_index]] call jna_fnc_arsenal;
	};
	...
	case "UpdateItemAdd":{
		...
		_ctrlList = _display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _index);
		_type = (ctrltype _ctrlList == 102);
		_cursel = if _type then{-1}else{lbCurSel _ctrlList};
		...
		["UpdateItemGui",[_display,_index,_l_found]] call jna_fnc_arsenal;
	};
	...
	case "CreateItem":{
		_display = _this select 0;
		_index = _this select 1;
		_item = _this select 2;			if(_item isEqualTo "")exitWith{};
		...
		_data = str [_item,_amount,_displayName];
		_lbAdd = 0;
		...
		//["UpdateItemGui",[_display,_index,_lbAdd]] call jna_fnc_arsenal;
	};
	...
	case "updateItemInfo": {
		...
		["ShowItemInfo",[_itemCfg]] call jna_fnc_arsenal;
		["ShowItemStats",[_itemCfg]] call jna_fnc_arsenal;
	};
	...
	case "SelectItem": {
		...
		switch _index do {
			...
			case IDC_RSCDISPLAYARSENAL_TAB_BACKPACK: {
				...
				if (_oldItem != _item) then {
					...
					if (_item != "") then{
						...
						[_index, _item] remoteExecCall ["jna_fnc_removeItem_Arsanal"];
					};
					{
						...
						if(_canAdd)then{
							...
							_indexItem = _x call jna_fnc_itemType;
							if!(_indexItem in [
							   IDC_RSCDISPLAYARSENAL_TAB_CARGOMAG,
							   IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL,
							   IDC_RSCDISPLAYARSENAL_TAB_CARGOTHROW,
							   IDC_RSCDISPLAYARSENAL_TAB_CARGOPUT
								...
								[_indexItem, _x] remoteExecCall ["jna_fnc_addItem_Arsanal"];
							};
						};
					} foreach _items;
					...
					//add back ammo, if possible
					{
						...
						if(_canAdd)then{
							...
							_indexItem = [_magazine,[
								IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL,
								IDC_RSCDISPLAYARSENAL_TAB_CARGOPUT,
								IDC_RSCDISPLAYARSENAL_TAB_CARGOTHROW
							]] call jna_fnc_itemType;
							[_indexItem, _magazine, _count] remoteExecCall ["jna_fnc_addItem_Arsanal"];
						};
					}forEach _magazines;
				...
				if (_oldItem != _item) then {
					...
					if (_item != "") then{
						...
						if (count _magazines > 0) then {
							...
							if([jna_dataList select IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL, _mag] call jna_fnc_ItemCount > 0)then{
								if((player canAddItemToUniform _mag)||(player canAddItemToVest _mag)||(player canAddItemToBackpack _mag))then{
									player addmagazine _mag;
									[IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL, _mag] remoteExecCall ["jna_fnc_removeItem_Arsanal"];
								}else{
			...
			case IDC_RSCDISPLAYARSENAL_TAB_HANDGUN: {
				...
				if (_oldItem != _item) then {
					...
					_updateMagazineList = [];
					{
						...
						[_indexItem, _magazine, _amount] remoteExecCall ["jna_fnc_addItem_Arsanal"];//TODO
					...
					//save and load attachments
					{
						...
						if(_idcList != -1)then{[_idcList, _x] remoteExecCall ["jna_fnc_addItem_Arsanal"]};
					}foreach _oldAttachments - _newAttachments;
					{
						...
						if(_idcList != -1)then{[_idcList, _x] remoteExecCall ["jna_fnc_removeItem_Arsanal"]};
			...
			case IDC_RSCDISPLAYARSENAL_TAB_ITEMBIPOD: {
				...
				_compatibleItems = _weapon call bis_fnc_compatibleItems;
				if not (({_x == _item} count _compatibleItems > 0) || _item isequalto "")exitwith{
					['TabSelectRight',[_display,_index]] call jna_fnc_arsenal;
				};
	...
	case "buttonCargo": {
		...
		if!(_loadOld isEqualTo _load)then{
			...
			if(_add > 0)then{
				...
				[_index, _item, _count] remoteExecCall ["jna_fnc_removeItem_Arsanal"];
			}else{
				_ctrlList lnbsettext [[_lbcursel,2],str (_amountOld - _count)];
		...
		["SelectItemRight",[_display,_ctrlList,_index]] call jna_fnc_arsenal;
	};
		...
		switch true do {
			case (_key == DIK_ESCAPE): {
				if (_inTemplate) then {
					...
					if (true) then {["buttonClose",[_display]] spawn jna_fnc_arsenal;} else {_display closedisplay 2;};
				};
				_return = true;
			};
			...
			case (_key in [DIK_RETURN,DIK_NUMPADENTER]): {
				...
				if (ctrlfade _ctrlTemplate == 0) then {
					if (BIS_fnc_arsenal_type == 0) then {
						["buttonTemplateOK",[_display]] spawn jna_fnc_arsenal;
					} else {
						["buttonTemplateOK",[_display]] spawn jna_fnc_arsenal;
					};
					_return = true;
			...
			//--- Save
			case (_key == DIK_S): {
				if (_ctrl) then {['buttonSave',[_display]] call jna_fnc_arsenal;};
			};
			//--- Open
			case (_key == DIK_O): {
				if (_ctrl) then {['buttonLoad',[_display]] call jna_fnc_arsenal;};
			};
	...
	case "buttonLoad": {
		_display = _this select 0;
		['showTemplates',[_display]] call jna_fnc_arsenal;
	...
	case "buttonTemplateDelete": {
		...
		["templateSelChanged",[_display]] call jna_fnc_arsenal;
	};
	...
	default	{
		//hint str ["ASRESNAL ERROR", _mode, _this];
	};
};
