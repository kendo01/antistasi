...
_base = "";
_base = [_mrkdestino] call AS_fnc_findBaseForCA;
_aeropuerto = "";
_aeropuerto = [_mrkdestino] call AS_fnc_findAirportForCA;
...
_nombreorig = [_aeropuerto] call AS_fnc_localizar;
	...
	for "_i" from 1 to _nveh do
		...
		if !(_tipoVeh in vehTank) then {
			...
			if !(_tipoVeh in vehTrucks) then
				...
				_tempInfo = [_veh, _grupoVeh, _soldados, _posorigen] call AS_fnc_fillCargo;
...
if (_aeropuerto != "") then
	{
	[_aeropuerto,60] spawn AS_fnc_addTimeForIdle;
