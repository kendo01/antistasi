...
if (_hasSPAA) then {
	...
	waitUntil {sleep 1; !(spawner getVariable _marker) OR ((3*count (allUnits select {((side _x == side_green) OR (side _x == side_red)) AND (_x distance _posMarker <= (_size max 100))}) == 0) AND ({alive _x} count units _groupGunners == 0))};
