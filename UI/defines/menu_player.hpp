...
#define ACT_VEH_UNLOCK "closeDialog 0; if !(isMultiplayer) then {hint 'It's unlocked already.'} else {if (player != stavros) then {[false] call AS_fnc_unlockVehicle} else {[true] call AS_fnc_unlockVehicle};};"
