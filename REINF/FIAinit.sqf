...
if !("ItemRadio" in unlockedItems) then {
	if ((_unit != leader _unit) && (_tipo != guer_sol_UN)) then {_unit unlinkItem "ItemRadio"};
};

if !(hayRHS) then {
	call {
		if (_tipo == guer_sol_LAT) exitWith {
		...
		if (_tipo == guer_sol_RFL) exitWith {
		...
		if (_tipo == guer_sol_GL) exitWith {
			[_unit,false,true,true,false] call randomRifle;
		};

		if (_tipo == guer_sol_R_L) exitWith {
			[_unit,true,true,true,true] call randomRifle;
			_skillSet = 1;
		};

		if (_tipo == guer_sol_UN) exitWith {
			{
		...
		if (_tipo == guer_sol_SL) exitWith {
			[_unit,false,true,true,false] call randomRifle;
			_skillSet = 2;
		};

		if (_tipo == guer_sol_TL) exitWith {
			[_unit,false,true,true,false] call randomRifle;
			_skillSet = 3;
		};

		if (_tipo == guer_sol_AR) exitWith {
			[_unit,false,true,true,false] call randomRifle;
			_skillSet = 4;
		};

		if (_tipo == guer_sol_MED) exitWith {
			_skillSet = 5;
			[_unit,true,true,true,false] call randomRifle;
		};

		if (_tipo == guer_sol_ENG) exitWith {
			_skillSet = 5;
			[_unit,true,true,true,false] call randomRifle;
		};

		if (_tipo == guer_sol_EXP) exitWith {
			_skillSet = 5;
			[_unit,true,true,true,false] call randomRifle;
			_unit addmagazine atMine;
		};

		if (_tipo == guer_sol_AM) exitWith {
			_skillSet = 5;
			[_unit,true,true,true,false] call randomRifle;
		};

		if (_tipo == guer_sol_MRK) exitWith {
