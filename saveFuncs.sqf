		...
		call {
			...
			if(_varName == 'posHQ') exitWith
				...
				petros setPos _varvalue;
					if (chopForest) then
					{
					if (!isMultiplayer) then {{ _x hideObject true } foreach (nearestTerrainObjects [position petros,["tree","bush"],20])} else {{ _x hideObjectGlobal true } foreach (nearestTerrainObjects [position petros,["tree","bush"],20])};
					};
				[] spawn buildHQ;
				};
