...
while {true} do
	...
	//if (debug) then {Dolvich globalChat format ["FPS Av:%1.FPS Lim:%2",fpsTotal / fpsCuenta, minimoFPS]};
	if (diag_fps < minimoFPS) then
		...
		//if (debug) then {Dolvich sideChat "Eliminados algunos civiles para incrementar FPS"};
