if (flag_savingClient) exitWith {hint "Your personal stats are being saved"};

if (!isDedicated) then {
	private ["_funds","_unit","_vehicle","_playerUID","_pList"];
	flag_flag_savingClient = true;

	_playerUID = getPlayerUID player;
	// Personal gear
	["gear_goggles",goggles player,_playerUID] call fn_savePlayerData;
	["gear_vest",vest player,_playerUID] call fn_savePlayerData;
	["gear_uniform",uniform player,_playerUID] call fn_savePlayerData;
	["gear_head",headGear player,_playerUID] call fn_savePlayerData;

	if (isMultiplayer) then {
		["stat_score",player getVariable ["score",0],_playerUID] call fn_savePlayerData;
		["stat_rank",rank player,_playerUID] call fn_savePlayerData;

		_funds = player getVariable ["dinero",0];
		{
			_unit = _x;
			if ((!isPlayer _unit) AND (alive _unit)) then {
				_funds = _funds + (server getVariable [(typeOf _unit),50]);
		...
		["pers_funds",_funds,_playerUID] call fn_savePlayerData;
		["pers_garage",personalGarage,_playerUID] call fn_savePlayerData;
	};

	// list of players active during the current campaign
	_pList = server getVariable ["campaign_playerList",[]];
	_pList pushBackUnique _playerUID;
	server setVariable ["campaign_playerList",_pList,true];
