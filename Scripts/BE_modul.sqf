...
fnc_BE_updateProgressBar = {
	...
	if (_cV > 1) then {
		...
		[format ["<t color='#1DA81D'>New FIA Skill Level: %1</t>", _v+1],0,0,4,0,0,4] remoteExec ["bis_fnc_dynamicText", Dolvich];
...
fnc_BE_buyUpgrade = {
	if (BE_currentStage == 3) exitWith {
		[petros,"hint","No further training available."] remoteExec ["commsMP",Dolvich];
	};
	...
	if ((server getVariable "resourcesFIA") > BE_currentPrice) then {
		...
		[petros,"hint","We don't have the resources."] remoteExec ["commsMP",Dolvich];
	};
};
...
fnc_BE_broadcast = {
	...
	[petros,"BE",_pI] remoteExec ["commsMP",Dolvich];
};
